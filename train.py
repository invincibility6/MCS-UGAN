...
from models.MCS_UGAN import test6_Generator, Discriminator
from utils.data_utils import Dataloader
from visdom import Visdom


parser = argparse.ArgumentParser()
parser.add_argument("--cfg_file", type=str, default="configs/train_parameters.yaml")
...
lambda_1, lambda_2= 7, 5
patch = (1, img_height//16, img_width//16)
# print("111@@@@@@@@@@@@@@")
# Initialize generator and discriminator
generator = test6_Generator()
...
# Optimizers
optimizer_G = torch.optim.Adam(generator.parameters(), lr=0.0001, betas=(lr_b1, lr_b2))
optimizer_D = torch.optim.Adam(discriminator.parameters(), lr=0.0001, betas=(lr_b1, lr_b2))
...
for epoch in range(epoch, num_epochs):
    for i, batch in enumerate(dataloader):
        ...
        loss_G = loss_GAN + lambda_1 * loss_1 + lambda_2 + loss_con
        loss_G.backward()
        optimizer_G.step()

        ## Print log
        if not i%50:
            img_fake1, img_fake2 = torch.split(imgs_fake, 1, 0)
            img_good_gt1, img_good_gt2 = torch.split(imgs_good_gt, 1, 0)
            img_fake1 = img_fake1.squeeze()
            img_fake2 = img_fake2.squeeze()
            img_distorted1 = img_good_gt1.squeeze()
            img_distorted2 = img_good_gt2.squeeze()
            # viz.image(img_fake1,
            #           win='image_history1',
            #           opts=dict(caption='img_fake1', store_History=True)
            #           )
            # viz.image(img_fake2,
            #           win='image_history2',
            #           opts=dict(caption='img_fake2', store_History=True)
            #           )
            # viz.image(img_good_gt1,
            #           win='image_history3',
            #           opts=dict(caption='img_good_gt1', store_History=True)
            #           )
            # viz.image(img_good_gt2,
            #           win='image_history4',
            #           opts=dict(caption='img_good_gt2', store_History=True)
            #           )
